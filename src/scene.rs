...
use pathfinder_content::{
    dash::OutlineDash, fill::FillRule, outline::Outline, pattern::{Image, Pattern}, stroke::OutlineStrokeToFill
};
...
use crate::{backend, font::FontRc};

use super::{FontEntry, TextSpan, DrawMode, Backend, Fill, Cache};
use pdf::font::Font as PdfFont;
use pdf::error::PdfError;
use std::sync::Arc;
use font::{Encoder, Glyph, pathfinder_impl::PathBuilder};
...
impl<'a> Backend for SceneBackend<'a> {
    ...
    fn get_font(&mut self, font_ref: &MaybeRef<PdfFont>, resolve: &impl Resolve) -> Result<Option<Arc<FontEntry<OutlineBuilder>>>, PdfError> {
        self.cache.get_font(font_ref, resolve)
    }
    fn add_text(&mut self, span: TextSpan<OutlineBuilder>, clip: Option<Self::ClipPathId>) {}
